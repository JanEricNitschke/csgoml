...
class TrajectoryPredictor:
    ...
    def position_state_distance(
        self, position_array_1, position_array_2, distance_type="geodesic"
    ):
        """Calculates a distance between two game states based on player positions

        Args:
            position_array_1: Numpy array from a set first index from an array as generated by __modify_data_frame_shape with configuration == "positions"
            position_array_2: Numpy array from a set first index from an array as generated by __modify_data_frame_shape with configuration == "positions"
            Shape of position_arrays is (5, 3, 2|1) with the first index indicating the player, the second the coordinate and the third the team
            distance_type: String indicating how the distance between two player positions should be calculated. Options are "geodesic", "graph" and "euclidian"
        ...
        # logging.info(self.map_name)
        areas = {1: defaultdict(dict), 2: defaultdict(dict)}
        for team in range(position_array_1.shape[-1]):
            for player in range(position_array_1.shape[0]):
                areas[1][team][player] = find_closest_area(
                    self.map_name, position_array_1[player, :, team]
                )["areaId"]
                areas[2][team][player] = find_closest_area(
                    self.map_name, position_array_2[player, :, team]
                )["areaId"]
        # logging.info(areas)
        for team in range(position_array_2.shape[-1]):
            side_distance = float("inf")
            for mapping in itertools.permutations(
                range(position_array_2.shape[0]), position_array_2.shape[0]
            ):
                cur_dist = 0
                for player1, player2 in enumerate(mapping):
                    if distance_type == "euclidian":
                        this_dist = math.sqrt(
                            (
                                position_array_1[player1][0][team]
                                - position_array_2[player2][0][team]
                            )
                            ** 2
                            + (
                                position_array_1[player1][1][team]
                                - position_array_2[player2][1][team]
                            )
                            ** 2
                            + (
                                position_array_1[player1][2][team]
                                - position_array_2[player2][2][team]
                            )
                            ** 2
                        )
                        ...
                        logging.error(
                            "Got invalid distance type %s. Valid options are 'geodesic', 'graph' and 'euclidian'.",
                            distance_type,
                        )
                        sys.exit()
                    cur_dist += this_dist
    ...
    def token_state_distance(self, token_array_1, token_array_2, distance_type):
        ...
        Args:
            ...
            distance_type: String indicating how the distance between two player positions should be calculated. Options are "geodesic", "graph" and "edit_distance"
    ...
    def trajectory_distance(
        ...
        for time in tqdm(range(length)):
            if len(trajectory_array_1.shape) > 2.5:
                distance += self.position_state_distance(
                ...
                distance += self.token_state_distance(
                    trajectory_array_1[time]
                    if time in range(len(trajectory_array_1))
                    else trajectory_array_1[-1],
                    trajectory_array_2[time]
