"use strict";
const de_ancient = ['SideEntrance', 'BombsiteA', 'TSpawn', 'CTSpawn', 'House', 'Tunnel', 'TSideUpper', 'Water', 'Outside', 'Middle', 'MainHall',
    'Alley', 'Ruins', 'SideHall', 'TopofMid', 'TSideLower', 'Ramp', 'BombsiteB', 'BackHall'].sort().concat(['None'])
const de_cache = ['Entrance', 'BombsiteA', 'LongA', 'BombsiteB', 'Garage', 'Middle', 'LongHall', 'Catwalk', 'TSpawn', 'Quad', 'Warehouse',
    'Forklift', 'Dumpster', 'Window', 'Rafters', 'Back', 'CTSpawn', 'StorageRoom', 'ARamp', 'Truck', 'Squeaky', 'Roof', 'Ducts'].sort().concat(['None'])
const de_cbble = ['BPlatform', 'Courtyard', 'BombsiteA', 'CTSpawn', 'BombsiteB', 'ARamp', 'LowerTunnel', 'Tunnels', 'Patio', 'Balcony', 'UpperTunnel',
    'LongA', 'Hut', 'Underpass', 'Connector', 'TunnelStairs', 'TRamp', 'Catwalk', 'Hay', 'TSpawn', 'SideDoor', 'TMain', 'SnipersNest'].sort().concat(['None'])
const de_dust2 = ['BombsiteA', 'LongDoors', 'BombsiteB', 'BDoors', 'OutsideTunnel', 'OutsideLong', 'ExtendedA', 'Hole', 'Catwalk', 'Side', 'TSpawn',
    'UpperTunnel', 'Short', 'TRamp', 'Middle', 'UnderA', 'ARamp', 'LongA', 'TunnelStairs', 'TopofMid', 'ShortStairs', 'CTSpawn', 'Pit', 'Ramp',
    'MidDoors', 'LowerTunnel'].sort().concat(['None'])
const de_inferno = ['CTSpawn', 'BombsiteA', 'TRamp', 'TSpawn', 'LowerMid', 'TopofMid', 'Quad', 'Upstairs', 'BombsiteB', 'Banana', 'Ruins', 'Middle',
    'BackAlley', 'Apartments', 'Graveyard', 'SecondMid', 'Balcony', 'Pit', 'Arch', 'Bridge', 'Underpass', 'Library', 'Deck', 'Kitchen'].sort().concat(['None'])
const de_mirage = ['CTSpawn', 'BombsiteA', 'TRamp', 'TicketBooth', 'TopofMid', 'Shop', 'Apartments', 'BombsiteB', 'PalaceAlley', 'TSpawn', 'Catwalk',
    'House', 'SnipersNest', 'Jungle', 'Tunnel', 'BackAlley', 'Middle', 'PalaceInterior', 'TunnelStairs', 'Ladder', 'Stairs', 'SideAlley', 'Scaffolding',
    'Truck', 'Connector', 'Balcony'].sort().concat(['None'])
const de_nuke = ['Hut', 'Roof', 'Outside', 'BombsiteB', 'Ramp', 'TSpawn', 'BombsiteA', 'Decon', 'Silo', 'Lobby', 'Vending', 'Garage', 'CTSpawn', 'Squeaky',
    'Vents', 'Rafters', 'HutRoof', 'Tunnels', 'LockerRoom', 'Observation', 'Secret', 'Heaven', 'Hell', 'Mini', 'Admin', 'Catwalk', 'Crane', 'Trophy',
    'Control'].sort().concat(['None'])
const de_overpass = ['BackofA', 'LowerPark', 'BombsiteA', 'Connector', 'Canal', 'Tunnels', 'Fountain', 'UpperPark', 'Walkway', 'Water', 'StorageRoom',
    'SnipersNest', 'Lobby', 'Construction', 'Playground', 'Restroom', 'Pipe', 'TSpawn', 'Alley', 'UnderA', 'BombsiteB', 'TStairs', 'SideAlley', 'Bridge',
    'Pit', 'Stairs'].sort().concat(['None'])
const de_train = ['Tunnel2', 'TStairs', 'BombsiteA', 'TMain', 'Ivy', 'Kitchen', 'BombsiteB', 'CTSpawn', 'Tunnel', 'LadderTop', 'BackofB', 'LockerRoom',
    'Connector', 'TSpawn', 'Tunnel1', 'LadderBottom', 'SnipersNest', 'Scaffolding', 'Alley', 'Dumpster', 'BPlatform', 'Tunnels', 'PopDog',
    'ElectricalBox'].sort().concat(['None'])
const de_vertigo = ['Pit', 'Side', 'BombsiteA', 'BombsiteB', 'Tunnels', 'ARamp', 'Elevator', 'TSpawn', 'BackDoor', 'BackofA', 'Mid', 'LadderTop',
    'Bridge', 'APlatform', 'Scaffolding', 'LadderBottom', 'CTSpawn', 'BackofB', 'TCorridorUp', 'TopofMid', 'Window', 'Crane'].sort().concat(['None'])
...
const kill_weapons = ["CZ75 Auto", "Desert Eagle", "Dual Berettas", "Five-SeveN", "Glock-18",
    ...
    "Galil AR", "M4A1", "M4A4", "SG 553", "AWP", "G3SG1", "SCAR-20", "SSG 08", "Smoke Grenade", "Flashbang",
    "HE Grenade", "Incendiary Grenade", "Molotov", "Decoy Grenade", "Knife", "Zeus x27"]
...
function on_load() {
    ...
    for (const [key, value] of maps_map) {
        ...
        const component = document.getElementById("map_component")
    ...
    for (const [key, value] of weapons_map) {
        ...
        const component_name = key.split("_")[0].toLowerCase() + "_component"
        const component = document.getElementById(component_name)
...
function select_map(mySelect) {
    const currentValue = mySelect.value
    const map_hull = document.getElementById("map_hull")
...
function removeElement(e) {
    ...
    addBtn.onclick = AddElement;
    addBtn.setAttribute("class", "addBtn")
    const element_id = e.target.parentNode.parentNode.id
    const div = element_id.slice(0, element_id.lastIndexOf('_'))
...
function adjust_start_max(end_number) {
    ...
    const start_input = document.getElementById("start_second")
    start_input.setAttribute("max", parseInt(value) - 1)
}

function fill_list_with_children_text(my_list_id) {
    const my_list = []
    const children = document.getElementById(my_list_id).children
...
async function collect_query_input() {
    ...
    const use_weapons_classes_Kill = document.querySelector('input[name="Kill_weapons_classes"]:checked').value
    let Kill_classes_q = []
    let Kill_weapons_q = []
    ...
    let CT_classes_forbidden_q = []
    let CT_classes_allowed_q = []
    let CT_weapons_forbidden_q = []
    let CT_weapons_allowed_q = []
    const use_weapons_classes_CT = document.querySelector('input[name="CT_weapons_classes"]:checked').value
    ...
    let T_classes_forbidden_q = []
    let T_classes_allowed_q = []
    let T_weapons_forbidden_q = []
    let T_weapons_allowed_q = []
    const use_weapons_classes_T = document.querySelector('input[name="T_weapons_classes"]:checked').value
    ...
    const event_data = {
        "map_name": map_name,
        "weapons": {
            "Kill": Kill_weapons_q, "T": { "Allowed": T_weapons_allowed_q, "Forbidden": T_weapons_forbidden_q },
            "CT": { "Allowed": CT_weapons_allowed_q, "Forbidden": CT_weapons_forbidden_q }
        },
        "classes": {
            "Kill": Kill_classes_q, "T": { "Allowed": T_classes_allowed_q, "Forbidden": T_classes_forbidden_q },
            "CT": { "Allowed": CT_classes_allowed_q, "Forbidden": CT_classes_forbidden_q }
        },
